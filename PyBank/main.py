...
Prev_Revenue = 1088983 #will use to calculate the revenue change for each month
...
with open(File_to_Load) as Revenue_Data:
    ...
    for row in reader:
        ...
        #Calculate the greatest increase using an array
        if (Revenue_Change > Greatest_Increase[1]):
            Greatest_Increase[0] = row["Date"]
            Greatest_Increase[1] = Revenue_Change

        #Calculate the greatest decrease using an array, it should calculate the lowest value
        if (Revenue_Change < Greatest_Decrease[1]):
            Greatest_Decrease[0] = row["Date"]
            Greatest_Decrease[1] = Revenue_Change 

#calculate the average revenue change
Revenue_Avg = sum(Revenue_Change_List) / (len(Revenue_Change_List) - 1)
...
with open(File_to_Output.txt, "w") as Txt_File:
